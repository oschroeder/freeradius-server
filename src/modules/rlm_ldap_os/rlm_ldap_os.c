...
read_mappings(ldap_instance* inst)
{
	...
	while (fgets(buf, sizeof buf, mapfile)!=NULL) {
		...
		if (token_count == 3) {
			...
			ptr = opstring;
			operator = gettoken((const char**)&ptr, buf, sizeof(buf));
...
static VALUE_PAIR *make_multipairs ( const char* source, FR_TOKEN operator )
{
	char	*idx, *values;
	char	buffer[1024];
	char	token[256];
	...
	strncpy (buffer, source, 1024);
	values = buffer;
	idx = strchr (values, ',');
	if (idx == 0)
		idx = values + strlen (values);
	while ( (idx - values) > 0 )
	{
		while (*values == ' ')
			values++; /* skip leading spaces */
		/* copy attr=value */
		cnt = idx - values;
		strncpy (token, values, cnt);
		values[cnt] = '\0';
		token[cnt] = '\0';
		values = idx+1;
		/* find attr_value */
		if ( (idx = strchr (token, '=')) == 0)
		{
			radlog(L_ERR, "  parsing token  %s failed.", token);
			return (pairlist);
		}
		/* copy attr_name */
		cnt = idx - token;
		strncpy (attr_name, token, cnt);
		attr_name[cnt] = '\0';
		cnt--;
		while (attr_name[cnt] == ' ')
		...
		while ((*idx == ' ') || (*idx == '"'))
			idx++;
		cnt = strlen (idx);
		while ((cnt > 0) && ((idx[cnt-1] == ' ') || (idx[cnt-1] == '"')))
		{
			idx[cnt] = '\0';
			cnt--;	/* skip trailing spaces */
		}
		strncpy (attr_value, idx, cnt);
		attr_value[cnt] = '\0';
		DEBUG ("     ##### parsed '%s' = '%s'", attr_name, attr_value);
		/* create value pair */
		newpair = pairmake (attr_name, attr_value, operator);
		if (newpair == NULL) {
			radlog(L_ERR, "  Failed to create the pair: %s", fr_strerror());
			return (pairlist);
		}
...
static int ldap_authorize(void *instance, REQUEST * request)
{
	...
	vp_tmp = pairfind(request->packet->vps, PW_LDAP_BASEDN);
	if (vp_tmp == NULL) {
		DEBUG2 ("  [%s] basedn not found in request, using config", inst->xlat_name);
		...
		DEBUG2 ("  [%s] basedn found in request, using it", inst->xlat_name);
		if (!radius_xlat(basedn, sizeof(basedn), vp_tmp->vp_strvalue,
				 request, ldap_escape_func)) {
			radlog(L_ERR, "  [%s] unable to create basedn.\n", inst->xlat_name);
			return RLM_MODULE_INVALID;
		}
	...
	vp_tmp = pairfind(request->packet->vps, PW_LDAP_MULTI_CHECK);
	if (vp_tmp != NULL) {
		DEBUG2 ("  [%s] searching for multi-check-attribute '%s'", inst->xlat_name, vp_tmp->vp_strvalue);
		tmp_vals = ldap_get_values(conn->ld, msg, vp_tmp->vp_strvalue);
		if (tmp_vals != NULL) {
			for (i = 0; tmp_vals[i] != NULL; i++) {
				DEBUG2 ("    [%s] multi-check-attribute found in LDAP:    '%s'", inst->xlat_name, tmp_vals[i]);
				pairadd (check_pairs, make_multipairs (tmp_vals[i], T_OP_SET));

			}
			DEBUG2 ("  [%s] multi-check-attribute parsing done.", inst->xlat_name);
		}
		else {
			DEBUG2 ("  [%s] multi-check-attribute '%s' empty in LDAP response", inst->xlat_name, vp_tmp->vp_strvalue);
		}
	}

	RDEBUG("looking for reply items in directory...");

	vp_tmp = pairfind(request->packet->vps, PW_LDAP_MULTI_REPLY);
	if (vp_tmp != NULL) {
		DEBUG2 ("  [%s] searching for multi-reply-attribute '%s'", inst->xlat_name, vp_tmp->vp_strvalue);
		if ( (tmp_vals = ldap_get_values(conn->ld, msg, vp_tmp->vp_strvalue)) != NULL) {
			for (i = 0; tmp_vals[i] != NULL; i++) {
				DEBUG2 ("    [%s] multi-reply-attribute found in LDAP:    '%s'", inst->xlat_name, tmp_vals[i]);
				pairadd (reply_pairs, make_multipairs (tmp_vals[i], T_OP_SET));
			}
			DEBUG2 ("  [%s] multi-reply-attribute parsing done.", inst->xlat_name);
		}
		else {
			DEBUG2 ("  [%s] multi-check-attribute '%s' empty in LDAP response", inst->xlat_name, vp_tmp->vp_strvalue);
		}
